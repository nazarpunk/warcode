...
import {PawDrawDocument} from './document'
import PawDrawEdit from './model/edit'
import {
    CancellationToken,
    CustomDocumentBackup,
    CustomDocumentBackupContext, CustomDocumentEditEvent,
    CustomEditorProvider, Disposable, EventEmitter, ExtensionContext,
    Uri,
    Webview, WebviewPanel, workspace
} from 'vscode'

export class PawDrawEditorProvider implements CustomEditorProvider<PawDrawDocument> {
    ...
    private readonly _onDidChangeCustomDocument = new EventEmitter<CustomDocumentEditEvent<PawDrawDocument>>()
    public readonly onDidChangeCustomDocument = this._onDidChangeCustomDocument.event

    public saveCustomDocument(document: PawDrawDocument, cancellation: CancellationToken): Thenable<void> {
        return document.save(cancellation)
    }

    public saveCustomDocumentAs(document: PawDrawDocument, destination: Uri, cancellation: CancellationToken): Thenable<void> {
        return document.saveAs(destination, cancellation)
    }

    public revertCustomDocument(document: PawDrawDocument): Thenable<void> {
        return document.revert()
    }

    public backupCustomDocument(document: PawDrawDocument, context: CustomDocumentBackupContext, cancellation: CancellationToken): Thenable<CustomDocumentBackup> {
