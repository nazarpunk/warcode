...
export class ZincVisitor extends ParserVisitor {
    ...
    #comment(ctx: ZincCstNode) {
        ctx[ZincRule.end]?.map(item => this.visit(item));
        ctx[ZincRule.comment]?.map(item => this?.bridge?.mark(item, TokenLegend.jass_comment));
    }
    ...
    [ZincRule.zinc](ctx: ZincCstNode) {
        ...
        return ctx[ZincRule.root]?.map(item => this.visit(item));
    }

    [ZincRule.root](ctx: ZincCstNode) {
        this.#comment(ctx);

        const native = ctx[ZincRule.native_declare];
        if (native) return this.visit(native);

        const func = ctx[ZincRule.function_declare];
        if (func) return this.visit(func);
    }

    [ZincRule.end](ctx: ZincCstNode) {
        this.#comment(ctx);
    }

    [ZincRule.native_declare](ctx: ZincCstNode) {
        this.#comment(ctx);

        const b = this?.bridge;
        if (b) {
            //console.log(ctx[ZincRule.takes]?.[0].startColumn);

            b.mark(ctx[ZincRule.constant]?.[0], TokenLegend.jass_constant);
            b.mark(ctx[ZincRule.identifier]?.[0], TokenLegend.jass_function_native);
            b.mark(ctx[ZincRule.native]?.[0], TokenLegend.jass_native);
            b.mark(ctx[ZincRule.takes]?.[0], TokenLegend.jass_takes);
            b.mark(ctx[ZincRule.returns]?.[0], TokenLegend.jass_returns);
        }

        this.visit(ctx[ZincRule.function_args]!);
    ...
    [ZincRule.function_locals](ctx: ZincCstNode) {
        ...
        if (b) {
            ...
            const local = variable?.[ZincRule.local];
            if (!local) {
                const {type} = variable?.[ZincRule.typedname];
                if (type) {
                    b.diagnostics.push({
    ...
    [ZincRule.variable_declare](ctx: ZincCstNode) {
        ...
        const local = ctx[ZincRule.local]?.[0];
